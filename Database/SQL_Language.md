...
|함수|기능|
|---|---|
|NVL(expr1, expr2)|첫번째 파라미터 검사 수 null이 아니면 반환하고 null이면 두번째 파라미터 반환|
|NVL2(expr1, expr2, expr3)|첫번째가 null이면 두번째, 또 null이면 세번째 반환|
|NULLIF(expr1, expr2)|두 파라미터를 검사해서 같으면 null 반환, 다르면 첫번째 반환|
|COALESCE(expr1, expr2, ..., exprn)|n번째 파라미터가 null이 아니면 반환(null이 아닌 n번째 파라미터를 찾는 함수로 마지막 파라미터는 꼭 값이 있어야 함)|<br>
